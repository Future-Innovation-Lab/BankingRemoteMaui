...
namespace BankingRemoteMaui.Services
{
    public class BankingService : IBankingService
    {
        ...
        public BankingService(IHttpsClientHandlerService service)
        {
        ...
        {
            var response = await _client.GetAsync(new Uri(Settings.ServerAddress + Settings.ClientControllerRoute));
        ...
        public async Task<Client> UpdateClient(Client client)
        {
            ...
            var response = await _client.PutAsync(new Uri(Settings.ServerAddress + Settings.ClientControllerRoute), requestContent);
            if (response.IsSuccessStatusCode)
            {
        ...
        {
            var response = await _client.GetAsync(new Uri(Settings.ServerAddress + Settings.ClientControllerRoute)+"/"+id);
            if (response.IsSuccessStatusCode)
            {
                var content = await response.Content.ReadAsStringAsync();
        ...
        {
            var response = await _client.GetAsync(new Uri(Settings.ServerAddress + Settings.TransactionControllerRoute) + "/" + bankAccountId);
            if (response.IsSuccessStatusCode)
            {
                var content = await response.Content.ReadAsStringAsync();
        ...
        public async Task<Client> VerifyLogin(Auth auth)
        {
            ...
            var response = await _client.PostAsync(new Uri(Settings.ServerAddress + Settings.AuthControllerRoute), requestContent);
            if (response.IsSuccessStatusCode)
            {
                var responseContent = await response.Content.ReadAsStringAsync();
